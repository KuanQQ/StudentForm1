...
namespace TRYY
{
	class Student
	{
		...
		public Student(string name, int english, int chinese, int mathScore)
		{
			Name = name;
			if (string.IsNullOrWhiteSpace(name))
			{
				throw new ArgumentNullException("姓名不可為空白");
			}
			English = english;
			if (english<0|| english>100)
			{
				throw new ArgumentNullException("英文成績不可超過100分和小於0");
			}
			Chinese = chinese;
			if (chinese < 0 || chinese > 100)
			{
				throw new ArgumentNullException("國文成績不可超過100分和小於0");
			}
			MathScore = mathScore;
			if (mathScore < 0 || mathScore > 100)
			{
				throw new ArgumentNullException("數學成績不可超過100分和小於0");
			}
		}
